...
{
    if (cell.switcher.on) {
        [[V2TIMManager sharedInstance] addToBlackList:@[self.friendProfile.userID] succ:^(id arr){
            NSLog(@"123123------%@",arr);
        } fail:nil];
        [TCUtil report:Action_Addblacklist actionSub:@"" code:@(0) msg:@"addblacklist"];
    } else {
        [[V2TIMManager sharedInstance] deleteFromBlackList:@[self.friendProfile.userID] succ:^(id arr){
            NSLog(@"456456------%@",arr);
        } fail:nil];
...
- (void)tableView:(UITableView *)tableView willDisplayHeaderView:(UIView *)view forSection:(NSInteger)section {
    ...
    header.textLabel.textColor =[UIColor clearColor];
}
